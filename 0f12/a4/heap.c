...
void mfree(int* b, Heap* h) {
	...
	int posn = getPosn(b, h);
	// If segmentHead at position is true and block is allocated
	if (h->segmentHead[posn] && h->allocated[posn]) {
		// For segmentSize iterations, 
		for (int i = 0; i < h->segmentSize[posn]; i++) {
			// Set block to unallocated
			h->allocated[posn+i] = false;
		}
		// reset segmentHead to false
		h->segmentHead[posn] = false;
		// reset segmentSize to 1
		h->segmentSize[posn] = 1;
	}
}
